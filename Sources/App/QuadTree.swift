...
extension QuadTreeElement {
    ...
    @inlinable
    func distanceKilometers(latitude: Float, longitude: Float) -> Float {
...
extension GeocodingDatabase.GeoTree {
    ...
    public func knn<T: QuadTreeElement>(
        latitude: Float,
        longitude: Float,
        count: Int,
        maxDistanceKilometer: Float,
        elements: [Int32: T]
    ) -> [(id: Int32, distance: Float)] {
...
public final class DistanceQueue {
    ...
    public init(length: Int) {
        queue = [(Int32, Float)](repeating: (0, 1_000_000_000), count: length)
    }

    public func insert(id: Int32, priority: Float) {
        guard
            let pos = queue.firstIndex(where: {
